...
export function ParentFeesPaymentHub({
  fees,
  childName,
  isLoading,
  isRetryingVa,
  ...
  const showVa = fees?.virtualAccountEnabled;
  const va = fees?.virtualAccount;
  const onlineEnabled = fees?.onlinePaymentEnabled;
      ...
      <div className="grid gap-4 lg:grid-cols-5 lg:items-start">
        <div className="space-y-3 lg:col-span-3">
          {showVa && va ? (
            ...
            <div className="rounded-2xl bg-gradient-to-br from-[#1a2744] to-[#243352] p-5 text-white shadow-lg">
            ...
            <div className={`${ACADEMIC_UI.dataPanel} px-5 py-6`}>
              ...
              <p className="mt-2 text-[13px] leading-relaxed text-neutral-600">
                Dedicated accounts are not enabled for this school yet. Pay online or contact the bursar.
              </p>
            </div>
          )}

          {showVa ? (
            <p className="flex items-center gap-2 px-1 text-[11px] font-medium text-neutral-500">
              <span className="inline-flex size-5 items-center justify-center rounded-full bg-brand-100 text-[10px] font-bold text-brand-800">
                1
              </span>
              Recommended — no card needed, works from any bank app
            </p>
          ) : null}
        </div>
        ...
        <div className={`${ACADEMIC_UI.dataPanel} lg:col-span-2`}>
          <div className={`${ACADEMIC_UI.tableHeader} px-4 py-4 sm:px-5`}>
            ...
            <p className="mt-0.5 text-[12px] text-neutral-500">Card or bank via Paystack</p>
          </div>
          ...
          <div className="space-y-4 p-4 sm:p-5">
            ...
            {!onlineEnabled ? (
              <p className="text-[12px] text-neutral-500">Online payments are not enabled for this school.</p>
              ...
              <button
                ...
                onClick={onPayOnline}
              >
                {paying || isPayPending ? 'Redirecting…' : `Pay ${formatKobo(payAmountMinor)} with Paystack`}
              </button>
            )}
          </div>
        </div>
