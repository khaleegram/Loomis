...
import { PLATFORM_UI } from '@/lib/platform/platform-ui';

export interface ConsoleHeroStat {
  label: string;
  value: string;
  hint: string;
  icon: LucideIcon;
...
interface PlatformConsoleHeroProps {
  sectionLabel?: string;
  title: string;
  description: string;
...
export function PlatformConsoleHero({
  sectionLabel = 'Platform console',
  title,
  description,
  stats,
  actions,
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="mb-2 flex items-center gap-2">
              ...
              <p className={PLATFORM_UI.sectionLabel}>{sectionLabel}</p>
            </div>
            <h1 className={PLATFORM_UI.pageTitle} style={PLATFORM_UI.pageTitleStyle}>
              {title}
            </h1>
            <p className={PLATFORM_UI.pageDesc}>{description}</p>
          </div>
                ...
                <div key={stat.label} className="card overflow-hidden rounded-xl p-4 sm:p-5">
                  <div className="mb-3">
                    <span
                      ...
                      style={{ background: stat.gradient ?? SURFACES.kpi.g1 }}
                    >
