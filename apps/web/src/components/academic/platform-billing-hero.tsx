...
interface PlatformBillingHeroProps {
...
export function PlatformBillingHero({
  termLabel,
  yearLabel,
  systemCount,
  ...
  const stats = [
    ...
    },
    {
      ...
      hint: psfRateMinor !== null ? `${formatKobo(psfRateMinor)} per student` : 'Rate not configured',
      icon: Wallet,
      gradient: psfRateMinor === null ? SURFACES.kpi.g4 : SURFACES.kpi.g2,
    },
    {
      label: 'Snapshot date',
      value: isLoading ? '—' : snapshotDate ?? 'Not set',
      hint: 'Auto-snapshot when date is reached',
      icon: CalendarClock,
    ...
    },
    {
      ...
      hint: minimumTermCommitment !== null ? `MTC: ${minimumTermCommitment}` : 'Minimum term commitment not set',
      icon: ClipboardList,
      ...
      >
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <p className={ACADEMIC_UI.sectionLabel}>School owner · platform billing</p>
            <h1 className="text-foreground" style={ACADEMIC_PAGE_TITLE_STYLE}>
              Platform Billing
            </h1>
            <p className={ACADEMIC_UI.pageDesc}>
              Your billable student count is snapshotted automatically on the snapshot date. PSF
              obligations are created at snapshot time — not when school fees are paid.
            </p>
          ...
          <div className="flex w-full shrink-0 flex-wrap gap-2 lg:w-auto lg:justify-end">
            <Link href="/school/academic/attestations" className={ACADEMIC_UI.btnSecondary}>
              Billing snapshots
            </Link>
